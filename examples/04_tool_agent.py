...
def main() -> None:
    ...
    tool_node = ToolCallNode(executor=ToolExecutor(tools), next_action="prepare_final")
    ...
    print("agent-core tool agent. Type 'exit' to quit." if args.interactive else "")
    while True:
        if args.interactive:
        ...
        if stream and count_model_deltas(context) > delta_count_before:
            ...
            print(answer)
        print_context(context, args.context)
        print()
